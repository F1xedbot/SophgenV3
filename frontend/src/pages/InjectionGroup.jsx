...
import { getInjections, deleteInjections } from '../api';
import { ArrowLeft, Trash2 } from 'lucide-react';

const InjectionGroup = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await getInjections(limit, page * limit, func_name, searchQuery || undefined);
      setData(res.data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleDelete = async () => {
    ...
    try {
        await deleteInjections(selectedIds);
        setSelectedIds([]);
  ...
  const columns = [
    ...
    { key: 'modification', label: 'Modification', render: (row) => (
        <div className="truncate max-w-md" title={row.modification}>
            {row.modification}
        </div>
    )},
  ...
  return (
    <div className="space-y-6">
      ...
      <Table 
        ...
        onRowClick={(row) => navigate(`/injections/${row.id}`)}
